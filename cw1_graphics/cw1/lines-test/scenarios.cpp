...
// scenario 3 :comparing identical lines drawn in reverse order
...
// scenario 4 :no gaps between two connected lines that are drawn one after the other
TEST_CASE( "No gaps", "[scenarios]" ){
    ...
    SECTION( "horizontal" ){
		...
		auto const count = count_pixel_neighbours( surface );
		REQUIRE( 2 == count[1] );
		REQUIRE( count[2] > 0 );
		REQUIRE( 0 == count[0] );

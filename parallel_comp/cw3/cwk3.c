...
int main( int argc, char **argv )
{
    ...
    cl_mem deviceIn = clCreateBuffer( context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, (size_t)nRows * nCols * sizeof(float), hostMatrix, &status );
    if( status != CL_SUCCESS )
    {
        printf( "Could not create input buffer: Error %d.\n", status );
        free( hostMatrix );
    ...
    cl_mem deviceOut = clCreateBuffer( context, CL_MEM_WRITE_ONLY, (size_t)nRows * nCols * sizeof(float), NULL, &status );
    if( status != CL_SUCCESS )
    {
        printf( "Could not create output buffer: Error %d.\n", status );
    ...
    status = clSetKernelArg( kernel, 0, sizeof(cl_mem), &deviceIn );
    status = clSetKernelArg( kernel, 1, sizeof(cl_mem), &deviceOut );
    ...
    status = clEnqueueReadBuffer( queue, deviceOut, CL_TRUE, 0, (size_t)nRows * nCols * sizeof(float), hostMatrix, 0, NULL, NULL );
    if( status != CL_SUCCESS )
    {
